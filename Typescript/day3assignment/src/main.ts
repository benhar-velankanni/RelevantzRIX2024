...
stringStorage.addItem('Python');
stringStorage.removeItem('JavaScript');
...
//extended generic class for perishable items
...
class PerishableItem<T> extends Item<T>{
  ...
  displayInfo(): string {
      ...
      return `${super.displayInfo()}, Expiry Date: ${this.expiryDate.toDateString()} (${expiryStatus})`;
  }
}

//usage Example

const item1 = new Item<number>(100,'Laptop', 50000);
console.log(item1.displayInfo());

const perishableItem = new PerishableItem<number>(101, 'Milk', 50, new Date('2025-04-25'));
console.log(perishableItem.displayInfo());