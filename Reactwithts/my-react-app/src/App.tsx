...
const ContactManagement: React.FC = () => {
  ...
  useEffect(() => {
    axios.get<any[]>('http://localhost:5010/Contact')
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (contact.id) {
      axios.put<any>(`http://localhost:5010/Contact/${contact.id}`, contact)
        .then(response => {
      ...
      axios.post<any>('http://localhost:5010/Contact', contact)
        .then(response => {
  ...
  const handleDelete = (id: string) => {
    axios.delete(`http://localhost:5010/Contact/${id}`)
